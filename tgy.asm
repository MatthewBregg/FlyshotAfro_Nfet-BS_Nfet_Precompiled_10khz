...
; This is an experimental option to allow starting closed-loop flywheel drives with complementary
; modulation to reduce diode loss, and combat loss-localization on the high (or non-PWM) side devices
; and consequent high/uneven temps, particularly for locked-rotor conditions where a high phase current
; flows at a low voltage command.
; For the ACE 2, this could be about 3/4 volt of drop and 30A with 80-90% OFF duty and consequently
; quite a few watts being dissipated.
; However, we don't want to enable regeneration current to flow. Without current sensors, it is
; difficult to do full-time complementary modulation of any sort while still being able to command
; zero phase current (and torque). Since the governor dithers voltage command rather crudely between
; steps, hitting it with COMP_PWM=1 and without this causes Problems.
; This thus mutes complementary PWM as soon as the governor is hit the first time during a run cycle.
; This should work well for a flywheel drive situation by complement-modulating throughout acceleration
; (as well as during any case that results in STARTUP being set such as a stall).

; High steady state loads at super low speeds for a given config. will run with some diode conduction,
; so this is not a magic bullet.

.equ	COMP_PWM_SELECTIVE	= 1

; Remember to check your dead time. This includes making sure any board .inc file doesn't clobber the below!
...
.equ	MOTOR_DEBUG	= 1	; Output sync pulses on MOSI or SCK, debug flag on MISO
...
.equ	TIMING_SCPWM	= 0x1000 ; Enable COMP_PWM_SELECTIVE below this speed

.equ	TIMING_RANGE3	= 0x1000 ; 1024us per commutation (V/Hz ramp ends here at 100% duty)
.equ	TIMING_MAX	= 0x0080 ; Safety governor (no less than 0x0080, 312500eRPM)

.equ	TIMEOUT_START	= 24000	; Timeout per commutation for ZC during starting
...
.equ	START_MOD_INC	= 32	; (stock 4) Start power modulation step count increase (wraps in a byte)
.equ	START_MOD_LIMIT	= 12	; (stock 24) Value at which power is reduced to avoid overheating
.equ	START_FAIL_INC	= 15	; start_tries step count increase (wraps in a byte, upon which we disarm)

.equ	ENOUGH_GOODIES	= 1	; This many start cycles without timeout will transition to running mode (tm4 experimental 05-01-18 - stock 12)
...
.def	flags1		= r17	; state flags
	...
	.equ	MUTE_CPWM	= 3	; Skip CPWM while set (for reasons other than short off time) (torukmakto4)
...
;----torukmakto4------
.def	governor_l	= r19		; governor low
;---------------------
...
falling_edge:
		...
		rjmp	falling_edge_isdigital	; skip over digital handler

falling_digital:
		; digital input handler, should end up here on puls < MIN_RC_PULS
		ldi	i_temp1, 0xa0	; or low(250 * CPU_MHZ)
		cp	rx_l, i_temp1
		ldi	i_temp1, 0x0f	; or high(250 * CPU_MHZ)
		cpc	rx_h, i_temp1
		brcs	digital_shift0
		; fall through: 250 < puls < MIN_RC_PULS us is a HIGH bit
digital_shift1:
		lds	i_temp1, dib_l
		lds	i_temp2, dib_h
		lsl	i_temp1
		rol	i_temp2		; Shift left
		inc	i_temp1		; add 1
		sts	dib_l, i_temp1
		sts	dib_h, i_temp2
		rjmp	rcpint_exit	; Do not set EVAL_RC

digital_shift0:
		lds	i_temp1, dib_l
		lds	i_temp2, dib_h
		lsl	i_temp1
		rol	i_temp2		; Shift left (leave 0 in lsb)
		sts	dib_l, i_temp1
		sts	dib_h, i_temp2		
		rjmp	rcpint_exit	; Do not set EVAL_RC

falling_edge_isdigital:
		; Regular pulse range (also: frame terminator) or digital input?
		; cpi2	rx_l, rx_h, MIN_RC_PULS * CPU_MHZ, i_temp1	; is pulse shorter than MIN_RC_PULS? (This is normally, ~768)
		
		ldi     i_temp1, 0x00   ; or low(768 * CPU_MHZ)
                cp      rx_l, i_temp1
                ldi     i_temp1, 0x30   ; or high(768 * CPU_MHZ)
                cpc     rx_h, i_temp1


		brcs	falling_digital					; yes, digital input
		; otherwise, set EVAL_RC (buffer will be cleared by evaluate_rc by doing this)
		; leaving rx_l and rx_h as pulse length for evaluate_rc and fall through to exit
...
;		.endif
		...
		.endif
		.if USE_ICP || USE_INT0
		rjmp	evaluate_rc_puls	; tm4 change; No calibration or aliasing BS
		.endif
...
evaluate_rc_puls:
		...
		; Evaluate digital frame.
		; Very crude check: is the highest bit 1?
		lds	temp1, dib_h
		subi	temp1, 0x80
...
puls_notdigital:
		...
		cpi2	temp1, temp2, MID_RC_PULS * CPU_MHZ, temp3	; We're looking for MID_RC_PULS < puls < MAX_RC_PULS for drive enable (ca. 1500us)
		brcs	puls_zero					; Branch if enable is valid and false
		ldi2	YL, YH, MAX_POWER				; if enable then peg to 100% voltage command (For now until we do digital voltage command too)
		rjmp	rc_duty_set

puls_zero:	clr	YL						; disable, set 0% duty
		clr	YH
		rjmp	rc_duty_set
...
update_timing:
	...
	; Selectively enable complementary PWM based on timing.
	; The idea here is that we ought to be synchronously rectifying whenever we are also V/Hz limited,
	; including starting from 0 rpm, locked-rotor, and restarting from any speed that would impose V/Hz
	; limits normally.
	;
	; We do not unmute CPWM anywhere else. This should both ensure that it turns on quickly while enabling
	; a motor at low speed, and ensure that it doesn't enable under any case where its result is unpredictable,
	; and may be undesired braking and a bus voltage transient.
	;
	; This is skipped if sys_control is such that indicates startup mode or governor has been operated.

	; Be aware of where you set things. Something interesting with this particular iteration is that the
	; FlyShot governor code allows the longest timing (lowest speed) setpoint to be 0x7fff, so this is
	; inherently clear of the 0x8000 with TIMING_SCPWM = 0x1000.
	
	.if COMP_PWM_SELECTIVE
		mov	temp4, sys_control_h	; High byte
		cpi	temp4, (MAX_POWER-2)>>8 ; 
		brcs	governor_entry		; sys_control is definitely not MAX_POWER
		cpi3	temp1, temp2, temp3, TIMING_SCPWM * CPU_MHZ / 2, temp4
		brcs	governor_entry		; Past here, do not reenable
		cbr	flags1, (1<<MUTE_CPWM)	; Lower speed, reenable
governor_entry:
	.endif
	...
	.if COMP_PWM_SELECTIVE
		sbr	flags1, (1<<MUTE_CPWM)	; Stop complementary modulation to suppress regeneration
	.endif
...
service_governor:
                lds     temp4, governor_h
                cp      temp1, governor_l
                cpc     temp2, temp4
                cpc     temp3, ZH
                brcc    update_timing1          ; not hitting it, keep fuel on for now
        .if COMP_PWM_SELECTIVE
                sbr     flags1, (1<<MUTE_CPWM)  ; Stop complementary modulation to suppress regeneration
        .endif
	;	mov	temp1, governor_l	; Force timing (Experimental, helps damp overshoot perhaps?)
	;	lds	temp2, governor_h	; Experimental
                lsr     sys_control_h           ; limit by reducing power
                ror     sys_control_l
        ;----------------------------------
...
.endif
		...
		.if POWER_RANGE < 1700 * CPU_MHZ / 16			; 1700 or 700 - is a torukmakto4 change
		sbrs	flags1, STARTUP
...
set_new_duty_set:
	...
	.if COMP_PWM_SELECTIVE
		; Selective CPWM. Stealing this unused (Clashes with flyshot code) flag allows a separate CPWM off-switch
		; which can be updated anywhere i.e. in update_timing (or any other deviant purposes)
		sbrc	flags1, MUTE_CPWM	; Skip over jump if clear
		rjmp	set_new_duty21		; If set, skip unsetting SKIP_CPWM and comping overhead (T flag is 1 right now to skip)
	.endif
...
;-----torukmakto4-------------
; FlyShot: Initialize governor to a low speed setting so that any signalling failure not caught by the system selftest
; is ballistically safe.
                ldi     temp1, 0x23		; 0x23b6 = 5000rpm with 14 pole
                ldi     temp2, 0xb6		;        = about 36 fps with a standard Hy-Con
                mov     governor_l, temp2            
                sts     governor_h, temp1
; Thanks M. Bregg for suggesting the feature.
;-----------------------------
...
i_rc_puls1:	clr	rc_timeout
		cbr	flags1, (1<<EVAL_RC)+(1<<I2C_MODE)    ;    +(1<<UART_MODE) (torukmakto4: UART_MODE is hereby commandeered)
		sts	rct_boot, ZH
		sts	rct_beacon, ZH
i_rc_puls2:	wdr
		.if defined(HK_PROGRAM_CARD)
...
i_rc_puls_rx:	rcall	evaluate_rc_init
	...
	;	.endif
		.if USE_INT0 || USE_ICP
		mov	temp1, flags1
		andi	temp1, (1<<I2C_MODE) ; +(1<<UART_MODE) ;Commandeered flag as MUTE_CPWM for COMP_PWM_SELECTIVE --RCD 032220
		breq	i_rc_puls3
...
i_rc_puls3:
		...
		rcall	beep_f1
		rcall	beep_f1			;torukmakto4 edit
...
start_from_running:
		...
		ldi	temp1, 6		; (1 is torukmakto4 edit, stock 6) Do not enable FETs during first cycle to
